...
- **결제 완료** : 조회하는 시점에 결제상태가 완료(status: paid)인 거래를 조회합니다. (결제취소건 미포함)
- **결제 예정** : 조회하는 시점에 결제상태가 예정(status: ready)인 거래를 조회합니다.
...
- **결제 실패** : 조회하는 시점에 결제상태가 실패(status: failed)인 거래를 조회합니다.
- **결제 취소** : 조회하는 시점에 결제상태가 취소(status: cancelled)인 거래를 조회합니다.
- **승인 대기** : 조회하는 시점에 결제상태가 승인대기(status: pending)인 거래를 조회합니다.\
...
- **조회결과 항목 확인하기**
  ...
  - **결제요청 시각 / 상태승인 시각 :** 조회하신 기준시각 기준으로 적용된 시각.
