...
## **Shopify(쇼피파이)로 구축된 경우**

- Shopify(쇼피파이)는 정기결제를 지원하지 않습니다.
- Shopify(쇼피파이) 이용 시 글로벌 포트원 콘솔 계정 생성이 필요합니다.다.
- Shopify(쇼피파이) 플러그인은 국내 PG를 지원하며, 지원 PG 및 결제수단은 개발자 문서를 참고해 주세요.
- **Shopify(쇼피파이)는 SaaS 이용계약서 작성 이 후 이용이 가능합니다. (Shopify(쇼피파이))플러그인 이용요금 발생)**
  - 월거래금액의 0.5%
  - <Highlight text="월 거래금액에 *0.5%를 적용한 금액이 $500에 미치지 않을 경우, 최소 이용요금인 $500가 청구됩니다." />
  - 월 초에 청구되면 월 말까지 납부하셔야 합니다.
  - 환율은 월말 기준 환율로 계산되며, 이를 KRW로 환산한 금액이 청구됩니다.
