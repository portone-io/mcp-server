...
searchTags:
  ...
  - 갤럭시아 테스트
  - 갤럭시아 실연동
datetime: 2025-12-18T00:00:00.000Z
---
...
- V2 : 포트원 차세대 결제 모듈로 직관적인 인터페이스를 지원  (추후 지원예정)
...
- [포트원 관리자콘솔↗](https://admin.portone.io/) > 결제 연동 > 연동 정보 >  채널 관리 > + 채널 추가
  - 연동 모드 : 테스트 연동
  - 결제대행사 : 갤럭시아머니트리
  - 결제모듈 : 결제창 일반/정기 결제 및 API 수기/정기결제
...
- 채널 속성 : 결제
- PG상점아이디 (MID) 클릭
  - 결제창 방식 일반/정기결제 : M251105747 선택
  - API 방식 수기(키인)결제  : M251105772 선택
  - API 방식 빌링결제 : M251105773 선택
  - 컬쳐랜드 상품권, 게임문화상품권, 티머니 : M2103135 선택
  - 휴대폰 빌링결제 : M251205787 선택
...
- 채널 이름 : 상점 정보만으로는 채널(PG)의 성격을 파악하기 어려워 채널(PG)의 이름을 설정하는 용도로 구분하기 위한 필수 설정값으로 임의값으로 설정이 가능합니다(단, 숫자, 공백, 글자, \_, - 만 가능)
- 채널 속성 : 결제
- PG상점아이디 (MID) : 갤럭시아머니트리 으로부터 받은 상점아이디를 입력합니다.
- apiKey : 갤럭시아머니트리 으로부터 받은 apiKey를 입력합니다.
...
<Callout title="V1 갤럭시아머니트리 개발가이드 보러가기↗" />
