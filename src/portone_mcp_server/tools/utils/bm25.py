...
def calculate_bm25_scores(
    ...
    BM25 is a probabilistic relevance model used to rank documents based on their
    relevance to a given query. This implementation tokenizes the query and uses
    regex matching to find occurrences in documents.
    ...
    try:
        ...
        pattern = re.compile(escaped_query, re.IGNORECASE | re.MULTILINE)
    ...
    for path, doc in documents.items():
        ...
        # Include documents with matches even if score is 0
        # This happens when IDF is 0 (term appears in many documents)
        if path in term_frequencies:
